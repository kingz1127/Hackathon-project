...
// ✅ AUTOMATIC RECEIPT GENERATION FUNCTIONS - Add to your existing file
...
const generateAutomaticReceipt = async (payment, actionType = 'approval') => {
  try {
    console.log(`🔄 Generating automatic receipt for ${actionType}:`, payment._id);
    
    // Only generate receipts for payments with actual paid amount
    if (payment.amountPaid <= 0) {
      console.log('⏭️ No amount paid, skipping automatic receipt generation');
      return null;
    }
    ...
    if (existingReceipt) {
      console.log('📝 Updating existing receipt:', existingReceipt.receiptNumber);
    ...
    console.log('✅ Automatic receipt generated:', receipt.receiptNumber);
    return receipt;

  } catch (error) {
    console.error('❌ Error in automatic receipt generation:', error);
    throw error;
  }
};
...
const generateReceiptForNewPayment = async (paymentData) => {
  try {
    console.log('🔄 Generating receipt for new admin payment:', paymentData._id);
...
// // ✅ Set payment for student - FIXED VERSION
...
//     // ✅ FIX: Include studentName in the payment creation
//     const payment = new Payment({
//       studentId,
//       studentName: studentName, // ✅ ADD THIS - it's now required in the model
...
// ✅ Set payment for student - FIXED VERSION
// ✅ Set payment for student - UPDATED to keep payments as pending
// ✅ Set payment for student - WITHOUT transaction creation
router.post('/admin/set-payment', async (req, res) => {
  try {
    const { studentId, studentName, amount, description, dueDate, type } = req.body;
    ...
    // ✅ REMOVED: Transaction creation for payment assignment
...
// ✅ Edit payment with all fields - UPDATED for proper amount handling
router.put('/admin/edit-payment/:paymentId', async (req, res) => {
  try {
    ...
    // ✅ AUTOMATIC RECEIPT GENERATION for paid amounts
    let receipt = null;
    if (newAmountPaid > 0 && newAmountPaid !== oldAmountPaid) {
      try {
        receipt = await generateAutomaticReceipt(payment, 'admin_edit');
        console.log('✅ Automatic receipt generated during payment edit');
      } catch (receiptError) {
        console.error('❌ Automatic receipt generation failed:', receiptError);
      }
    }
...
// ✅ Update the receipt generation in financeRoutes.js

// ✅ Generate receipt function - COMPLETE VERSION with proper student data and balances
const generateReceipt = async (payment, amountPaid, paymentMethod = 'Online') => {
  try {
    ...
    // ✅ Calculate proper balances for the student
    const studentPayments = await Payment.find({ studentId: payment.studentId });
    const totalDue = studentPayments.reduce((sum, p) => sum + (p.amount || 0), 0);
    const totalPaid = studentPayments.reduce((sum, p) => sum + (p.amountPaid || 0), 0);
    const balanceRemaining = Math.max(0, totalDue - totalPaid);
    ...
    const receipt = new Receipt({
      ...
      tax: 0,
      total: amountPaid,
      balanceInfo: {  // ✅ Store balance information in receipt
        totalDue: totalDue,
        totalPaid: totalPaid,
        balanceRemaining: balanceRemaining
      }
    ...
    await receipt.save();
    console.log('✅ Receipt generated with complete info:', {
      receiptNumber: receipt.receiptNumber,
...
// ✅ NEW: Generate receipt for transaction
...
//       topStudents,
//       recentTransactions: enhancedTransactions, // ✅ Use enhanced transactions
...
/// ✅ Get financial overview for admin - FIXED TRANSACTIONS VERSION
router.get('/admin/overview', async (req, res) => {
  try {
    const students = await Student.find({ isActive: true });
    const payments = await Payment.find();
    
    // ✅ FIX: Get transactions with proper sorting and field mapping
    ...
    // ✅ FIX: Enhanced transactions with proper field mapping
    const enhancedTransactions = await Promise.all(
      transactions.map(async (transaction) => {
        ...
        return {
          ...
          description: transaction.description,
          amount: transaction.amount,
          date: transaction.date || transaction.createdAt, // ✅ Use date field
          type: transaction.type,
          status: transaction.status,
          transactionId: transaction.transactionId // ✅ Include transaction ID
        };
      })
    );
    ...
    console.log(`📊 Overview: ${enhancedTransactions.length} transactions loaded`);
    res.json(overviewData);
  } catch (error) {
    console.error('Error fetching admin overview:', error);
    res.status(500).json({ message: 'Failed to fetch overview', error: error.message });
...
// ✅ Get all receipts with filtering (replace existing one)
router.get('/admin/receipts', async (req, res) => {
  try {
    ...
    const formattedReceipts = receipts.map(receipt => {
      // ✅ FIX: Ensure we have a valid date
      ...
      return {
        ...
        studentId: receipt.studentId,
        date: receiptDate,
        time: new Date(receiptDate).toLocaleTimeString(), // ✅ FIXED: Use Date object
        total: receipt.total,
        status: receipt.status,
        paymentMethod: receipt.paymentMethod,
        items: receipt.items || []
...
/// ✅ Get student financial overview - FIXED TRANSACTIONS VERSION
router.get('/student/:studentId/overview', async (req, res) => {
  try {
    ...
    // ✅ FIX: Get recent transactions for this student with proper sorting
    ...
    console.log(`📊 Found ${recentTransactions.length} transactions for student ${studentId}`);
    ...
    // ✅ FIX: Format transactions properly
    ...
    const studentOverview = {
      ...
      recentTransactions: formattedTransactions, // ✅ Use formatted transactions
...
// ✅ FIXED: Get student receipts - ENHANCED VERSION
router.get('/student/:studentId/receipts', async (req, res) => {
  try {
    ...
    console.log(`✅ Found ${receipts.length} receipts for student ${studentId}`);
...
router.post('/student/:studentId/make-payment', paymentLimiter, async (req, res) => {
  try {
    ...
    // ✅ ADD AUTOMATIC RECEIPT GENERATION HERE
    let receipt = null;
    try {
      receipt = await generateAutomaticReceipt(payment, 'student_payment');
      console.log('✅ Automatic receipt generated for student payment');
    } catch (receiptError) {
      console.error('❌ Automatic receipt generation failed:', receiptError);
...
/// ✅ Generate receipt from payment data - UPDATED with complete student info
  ...
  try {
    ...
    if (!receipt) {
      ...
      receipt = new Receipt({
        receiptNumber,
        studentId: payment.studentId,
        studentName: student.fullName,    // ✅ Use actual student name
        studentEmail: student.email,      // ✅ Include email
        studentCourse: student.course,    // ✅ Include course
        paymentId: payment._id,
    ...
    const enhancedReceipt = {
      ...
      studentId: receipt.studentId,
      studentEmail: receipt.studentEmail || student.email,    // ✅ Ensure email is included
      studentCourse: receipt.studentCourse || student.course, // ✅ Ensure course is included
      date: receiptDate,
...
// ✅ Get receipt by ID - FIXED VERSION that handles transaction IDs
router.get('/receipt/:receiptId', async (req, res) => {
  try {
    ...
    if (receipt) {
      ...
      // ✅ Return EXACT database data - no modifications
    ...
    // ✅ If no receipt found, check if it's a transaction ID
    console.log('🔍 No receipt found, checking if it\'s a transaction ID:', receiptId);
    const transaction = await Transaction.findById(receiptId);
    
    if (transaction) {
      console.log('✅ Found transaction, creating receipt from transaction data');
      ...
      console.log('✅ Created new receipt from transaction:', newReceipt.receiptNumber);
...
// ✅ Update existing receipts with student information
...
// ✅ Generate receipts for all existing payments
...
// ✅ Get transaction by ID - UPDATED VERSION with correct field mapping
router.get('/admin/transaction/:transactionId', async (req, res) => {
  try {
    ...
    // ✅ FIX: Return fields that match Transaction model
    res.json({
      id: transaction._id,
      transactionId: transaction.transactionId,
      studentId: transaction.studentId,
      ...
      type: transaction.type,
      status: transaction.status,
      date: transaction.transactionDate, // ✅ Use transactionDate from model
...
// ✅ Generate receipts for all existing transactions
...
// ✅ FIXED: Send receipt to student - WORKING VERSION
router.post('/admin/receipts/:receiptId/send', async (req, res) => {
  try {
    const { receiptId } = req.params;
    const { studentId, sendEmail = true } = req.body;
    
    console.log('📤 Sending receipt to student:', { receiptId, studentId, sendEmail });
    ...
    if (!receipt) {
      console.log('🔍 Receipt not found by ID, trying transaction ID...');
    ...
    if (sendEmail && student.email && student.email !== 'N/A') {
      try {
        ...
        console.log(`📄 Receipt details: ${receipt.receiptNumber} - $${receipt.total}`);
    ...
    console.log('✅ Receipt successfully sent to student:', student.fullName);
    ...
    console.error('❌ Error sending receipt to student:', error);
    res.status(500).json({ 
      success: false, 
...
// ✅ Sync receipts for a student
router.post('/student/:studentId/sync-receipts', async (req, res) => {
  try {
    const { studentId } = req.params;
    console.log('🔄 Syncing receipts for student:', studentId);
    ...
    if (student) {
      ...
      for (const receipt of receiptsByStudentName) {
        if (!receipt.studentId || receipt.studentId === 'N/A') {
          ...
          receipt.studentCourse = student.course;
          await receipt.save();
          console.log(`✅ Updated receipt ${receipt.receiptNumber} with student ID`);
        }
      }
    }
