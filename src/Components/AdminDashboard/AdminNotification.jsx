...
// Add this Pagination component before your main component
...
// Add this filtered finance data logic near your other filtered data
const filteredFinanceData = financeData.filter(submission => {
  const matchesSearch = 
    submission.studentName?.toLowerCase().includes(financeSearchTerm.toLowerCase()) ||
    submission.studentId?.toLowerCase().includes(financeSearchTerm.toLowerCase()) ||
    submission.paymentDescription?.toLowerCase().includes(financeSearchTerm.toLowerCase()) ||
    submission.amount?.toString().includes(financeSearchTerm);

  const matchesStatus = financeStatusFilter === 'all' || submission.status === financeStatusFilter;
  const matchesPaymentMethod = financePaymentMethodFilter === 'all' || 
    (submission.paymentMethod && submission.paymentMethod === financePaymentMethodFilter);
  ...
  // Add pagination data calculations
 const paginatedFinanceData = useMemo(() => {
  const startIndex = (currentFinancePage - 1) * ITEMS_PER_PAGE;
  const endIndex = startIndex + ITEMS_PER_PAGE;
  return filteredFinanceData.slice(startIndex, endIndex);
}, [filteredFinanceData, currentFinancePage]);

// Update total pages calculation
const financeTotalPages = Math.ceil(filteredFinanceData.length / ITEMS_PER_PAGE);

  const paginatedRegistrationData = useMemo(() => {
    const startIndex = (currentRegistrationPage - 1) * ITEMS_PER_PAGE;
    const endIndex = startIndex + ITEMS_PER_PAGE;
    return filteredRegistrationData.slice(startIndex, endIndex);
  }, [filteredRegistrationData, currentRegistrationPage]);

  // const financeTotalPages = Math.ceil(financeData.length / ITEMS_PER_PAGE);
  const registrationTotalPages = Math.ceil(filteredRegistrationData.length / ITEMS_PER_PAGE);

  // Add tab change handler to reset pagination
  // Update the handleTabChange function to reset filters when switching tabs
const handleTabChange = (tab) => {
  ...
  // Add this finance filters component near your other filter components
const renderFinanceFilters = () => (
  <div className={styles.filtersContainer}>
    <div className={styles.searchBox}>
      <input
        type="text"
        placeholder="Search by student name, ID, payment description, or amount..."
...
const updatePaymentStatus = async (submissionId, status, approvedAmount = null) => {
  ...
  // ✅ UPDATED: Prevent double approval and refresh data
const handleApprove = async () => {
  ...
  try {
    ...
    await updatePaymentStatus(submissionId, 'approved', adminNotes);
    
    // Refresh the finance data after successful approval
  ...
  // ✅ UPDATED: Prevent double decline and refresh data
const handleDecline = async () => {
  ...
  try {
    await updatePaymentStatus(submissionId, 'declined', adminNotes);
    
    // Refresh the finance data after successful decline
    await fetchSubscriptions();
...
const handleRegistrationStatusUpdate = async (status) => {
  ...
  if (success) {
    // Refresh registration data after successful status update
    await fetchRegistrationData();
    await fetchRegistrationStatistics();
  ...
  <div className={styles.financeSection}>
    ...
    <div className={styles.resultsInfo}>
      <p>Showing {Math.min(paginatedFinanceData.length, ITEMS_PER_PAGE)} of {filteredFinanceData.length} submissions (Page {currentFinancePage} of {financeTotalPages})</p>
    </div>

    {filteredFinanceData.length === 0 ? (
      <div className={styles.emptyState}>
        <div className={styles.emptyIcon}>💰</div>
        <h3>No Financial Submissions Found</h3>
        <p>Try adjusting your search or filters.</p>
      </div>
    ) : (
      <>
        <div className={styles.submissionsGrid}>
          {paginatedFinanceData.map((submission) => (
        ...
        {financeTotalPages > 1 && (
          <Pagination
            currentPage={currentFinancePage}
            totalPages={financeTotalPages}
            onPageChange={setCurrentFinancePage}
          />
        )}
      </>
    )}
        ...
        {activeTab === 'registration' && (
          <div className={styles.registrationSection}>
            ...
            <div className={styles.resultsInfo}>
              <p>Showing {Math.min(paginatedRegistrationData.length, ITEMS_PER_PAGE)} of {filteredRegistrationData.length} submissions (Page {currentRegistrationPage} of {registrationTotalPages})</p>
            </div>
            ...
            {filteredRegistrationData.length === 0 ? (
              ...
              <>
                <div className={styles.registrationsGrid}>
                  {paginatedRegistrationData.map((submission) => (
                ...
                {registrationTotalPages > 1 && (
                  <Pagination
                    ...
                    onPageChange={setCurrentRegistrationPage}
                  />
                )}
              </>
            )}
